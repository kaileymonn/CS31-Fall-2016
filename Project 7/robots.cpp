...
void Player::moveOrAttack(int dir)
{
	...
	//use arena to write code
	int c_row = m_row;
	int c_col = m_col;

	if (m_arena->determineNewPosition(c_row, c_col, dir))
	{
		//is navigable
		if (m_arena->nRobotsAt(c_row, c_col) != 0)
		{
			m_arena->attackRobotAt(c_row, c_col, dir);
		}
		else
		{
			m_row = c_row;
...
bool Arena::determineNewPosition(int& r, int& c, int dir) const
{
	...
	//Didn't manage to move in any dir. Return false.
	return false;
}
...
bool Arena::attackRobotAt(int r, int c, int dir)
{
	...
	for (int i = 0; i < m_nRobots; ++i) {
		if (m_robots[i]->col() == c && m_robots[i]->row() == r) {
...
//  main()
///////////////////////////////////////////////////////////////////////////
void doBasicTests(); //Remove this line after completing test.
int main()
{
	// Create a game
